...
function makeScannerPopupHtml(scanToken) {
  return [
    ...
    '     ]},',
    /* accept only after two identical consecutive reads — a misread
       almost never produces the same wrong value twice */
    '    function(text){',
    '      text=String(text||"").trim();',
    '      if(!text||hasScanned)return;',
    '      if(text===lastT){streak++;}else{lastT=text;streak=1;}',
    '      if(streak<2)return;',
    '      sendResult(text);',
    '    },',
